...
class cudnn_gru:
    ...
    def __init__(self, num_layers, num_units, batch_size, input_size, keep_prob=1.0, is_train=None, scope=None):
        ...
        for layer in range(num_layers):
            ...
            param_fw = tf.get_variable("param_fw_layer_{}".format(layer), initializer=tf.random_uniform([gru_fw.params_size()], -0.1,0.1), validate_shape=False)
            # param_bw = tf.Variable(tf.random_uniform(
            #     [gru_bw.params_size()], -0.1, 0.1), validate_shape=False)
            param_bw = tf.get_variable("param_bw_layer_{}".format(layer), initializer=tf.random_uniform([gru_bw.params_size()], -0.1, 0.1), validate_shape=False)
            # init_fw = tf.Variable(tf.zeros([1, batch_size, num_units]))
            # init_bw = tf.Variable(tf.zeros([1, batch_size, num_units]))
            init_fw = tf.get_variable("init_fw_layer_{}".format(layer), initializer=tf.zeros([1, batch_size, num_units]))
            init_bw = tf.get_variable("init_bw_layer_{}".format(layer), initializer=tf.zeros([1, batch_size, num_units]))
...
def dense(inputs, hidden, use_bias=True, scope="dense"):
    with tf.variable_scope(scope):
        ...
        W = variable_on_cpu("W", [dim, hidden])
        ...
        if use_bias:
            ...
            b = variable_on_cpu("b", [hidden], initializer=tf.constant_initializer(0.1))
            res = tf.nn.bias_add(res, b)
        res = tf.reshape(res, out_shape)
        return res

def variable_on_cpu(name, shape, dtype=None, initializer=None):
